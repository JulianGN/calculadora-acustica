...
function CalculadoraArticle({ atributos, url, current }) {
  ...
  useEffect(() => {
    ...
    if (current?.ordem == atributos.ordem) setClasses("top-menu-item active");
    else setClasses("top-menu-item");
  }, [current]);

  return (
    <ContainerItem className={classes}>
      <img src={atributos.imagem} title={atributos.nome} alt={atributos.nome} />
